-- 일반_원수_직전3년연간경과보험료 (2021.07.28)
SELECT /*+ FULL(A) PARALLEL(A 8) */
       SUBSTR(CLG_YM,1,4) AS FY, RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD,
...
-- 일반_출재_직전3년연간경과보험료 (2021.07.28)
SELECT SUBSTR(CLG_YM,1,4) AS FY, RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD,
       SUM(T02_RN_ELP_PRM) AS RN_ELP_PRM
  FROM FDMTAG04002
  WHERE CLG_YM BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE(:ET_PARM02, 'YYYYMMDD'),-35),'YYYYMM') AND SUBSTR(:ET_PARM02,1,6)
  GROUP BY SUBSTR(CLG_YM,1,4), RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5);

-- 일반_원수_직전3년연간손해액 (2021.07.28)
SELECT SUBSTR(CLG_YM,1,4) AS FY, RRNR_DAT_DVCD, RRNR_DMFR_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD,
...
-- 일반_특약보종별_직전1년경과보험료 (2021.07.28)
SELECT D.RRNR_DAT_DVCD, D.RRNR_CTC_BZ_DVCD, D.PDC_CD, C.RRNR_TTY_CD,
	     SUM(D.ELP_PRM) AS OGL_ELP_PRM_1YR,
	     SUM(C.T02_RN_ELP_PRM) AS RN_ELP_PRM_1YR
	FROM (
	...
	-- 일반_특약보종별_직전1년손해액 (2021.07.28)
	SELECT D.RRNR_DAT_DVCD, D.RRNR_DMFR_DVCD, D.PDC_CD, C.RRNR_TTY_CD,
	       SUM(D.OGL_LOSS) AS OGL_LOSS_1YR,
	       SUM(C.RN_LOSS) AS RN_LOSS_1YR
		FROM (
...
-- 일반_특약보종별_경과보험료 (2021.07.28)
SELECT /*+ FULL(C) FULL(D) PARALLEL(C 8) PARALLEL(D 8) */
       D.RRNR_DAT_DVCD, D.RRNR_CTC_BZ_DVCD, D.PDC_CD, C.RRNR_TTY_CD,
	     SUM(D.ELP_PRM) AS OGL_ELP_PRM,
	...
	FROM (
					SELECT PLNO, RRNR_TTY_CD, SUM(T02_RN_ELP_PRM) AS T02_RN_ELP_PRM
						...
						WHERE EXISTS (
							...
							AND CLG_YM <= SUBSTR(:ET_PARM02,1,6)
						GROUP BY PLNO, RRNR_TTY_CD
			 ) C INNER JOIN (
					SELECT PLNO, RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD, SUM(ELP_PRM) AS ELP_PRM
					  FROM FDMTAG04001
