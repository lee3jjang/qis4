-- 일반_원수_미경과보험료 (2021.07.28)
SELECT /*+ FULL(A) PARALLEL(A 8) */ RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD, NTNL_CTRY_CD, INRI_RN_KND_CD,
...
-- 일반_출재_미경과보험료 (2021.07.28)
SELECT RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD, NTNL_CTRY_CD, RN_KND_CD, T02_RN_RINSC_CD,
       CASE WHEN CMSN_DVCD = '1' THEN '01' ELSE '02' END AS CMSN_DVCD,
       SUM(T02_LTPD_RN_URND_PRM) AS RN_URND_PRM
	FROM FDMTAG04002
	WHERE CLG_YM = SUBSTR(:ET_PARM02,1,6)
	GROUP BY RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5), NTNL_CTRY_CD, RN_KND_CD, T02_RN_RINSC_CD,
				   CASE WHEN CMSN_DVCD = '1' THEN '01' ELSE '02' END;

-- 일반_원수_직전1년경과보험료 (2021.07.28)
SELECT /*+ FULL(A) FULL(B) PARALLEL(A 8) PARALLEL(B 8) */
       RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD, NTNL_CTRY_CD, INRI_RN_KND_CD, NVL(B.CMSN_DVCD,'02') CMSN_DVCD,
	...
	GROUP BY RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5), NTNL_CTRY_CD, INRI_RN_KND_CD, NVL(B.CMSN_DVCD,'02');

-- 일반_출재_직전1년경과보험료 (2021.07.28)
SELECT RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD, NTNL_CTRY_CD, RN_KND_CD, T02_RN_RINSC_CD,
       CASE WHEN CMSN_DVCD = '1' THEN '01' ELSE '02' END AS CMSN_DVCD,
...
-- 일반_원수_예정보험료 (2021.07.28)
SELECT /*+ FULL(A) FULL(B) PARALLEL(A 8) PARALLEL(B 8) */
       RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD,
	     SUM(WNCR_CNV_EPCT_PRM) AS OGL_EPCT_PRM
	FROM (
  	SELECT DISTINCT PLNO, PYM_EPCT_DT, WNCR_CNV_EPCT_PRM
			FROM FDMTAG04003
  		WHERE 1=1
  	...
  	SELECT DISTINCT PLNO, ARC_INPL_CD, RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD
    	FROM FDMTAG04001
  ) B
  ON A.PLNO = B.PLNO
  GROUP BY RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, SUBSTR(ARC_INPL_CD,1,5);

-- 일반_출재_예정보험료 (2021.07.28)
WITH EPCT_RN_PRM AS (
...
SELECT /*+ FULL(A) FULL(B) PARALLEL(A 8) PARALLEL(B 8) */
       B.RRNR_DAT_DVCD, B.RRNR_CTC_BZ_DVCD, SUBSTR(B.ARC_INPL_CD,1,5) AS PDC_CD, A.T02_RN_RINSC_CD,
       SUM(A.WNCR_CNV_RN_PRM) AS RN_EPCT_PRM
	FROM EPCT_RN_PRM A
  LEFT JOIN (SELECT DISTINCT PLNO, ARC_INPL_CD, RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD FROM FDMTAG04002) B
  	ON A.PLNO = B.PLNO
  GROUP BY B.RRNR_DAT_DVCD, B.RRNR_CTC_BZ_DVCD, SUBSTR(B.ARC_INPL_CD,1,5), A.T02_RN_RINSC_CD
  HAVING SUM(A.WNCR_CNV_RN_PRM) != 0;

-- 일반_원수_개별추산액 (2021.07.28)
SELECT /*+ FULL(A) PARALLEL(A 8) */
       RRNR_DAT_DVCD, RRNR_DMFR_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD, NTNL_CTRY_CD, INRI_RN_KND_CD,
	...
	FROM FDMTTC04001 A
	WHERE CLG_YM = SUBSTR(:ET_PARM02,1,6)
	GROUP BY RRNR_DAT_DVCD, RRNR_DMFR_DVCD, SUBSTR(ARC_INPL_CD,1,5), NTNL_CTRY_CD, INRI_RN_KND_CD,
...
-- 일반_출재_개별추산액 (2021.07.28)
SELECT RRNR_DMFR_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD, NTNL_CTRY_CD, RN_KND_CD, T02_RN_RINSC_CD,
       CASE WHEN CMSN_DVCD = '1' THEN '01' ELSE '02' END AS CMSN_DVCD,
       SUM(RN_LTPD_OST_AMT) AS RN_OST_AMT
	FROM FDMTTC04002
	WHERE CLG_YM = SUBSTR(:ET_PARM02,1,6)
	GROUP BY RRNR_DMFR_DVCD, SUBSTR(ARC_INPL_CD,1,5), NTNL_CTRY_CD, RN_KND_CD, T02_RN_RINSC_CD,
           CASE WHEN CMSN_DVCD = '1' THEN '01' ELSE '02' END;

-- 일반_원수_직전3년보험금손조비 (2021.07.28)
SELECT RRNR_DAT_DVCD, RRNR_DMFR_DVCD, SUBSTR(ARC_INPL_CD,1,5) AS PDC_CD,
...
-- 일반_IBNR (2021.08.04)
SELECT CLG_YM, RRNR_DVCD, IBNR_KEY,
		   SUM(NVL(KRW_OS_AMT_E, 0)) AS KRW_OS_AMT_E
	FROM (
 ...
 GROUP BY CLG_YM, RRNR_DVCD, IBNR_KEY
 ORDER BY 2, 3;
...
-- 재보험자_KICS신용등급 (2021.07.28)
SELECT RINSC_CD, RINSC_NM, KICS_CRDT_GRCD
	FROM FDMTAT01106
	WHERE BSE_DT = :ET_PARM02;