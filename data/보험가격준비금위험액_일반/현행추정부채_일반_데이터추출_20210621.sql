...
WITH VALID_PLNO AS (
	SELECT PLNO
		...
		GROUP BY PLNO
		HAVING SUM(LTPD_URND_PRM) > 0
)

SELECT RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, NTNL_CTRY_CD, ARC_INPL_CD, INER_CHN_DVCD,
       COUNT(DISTINCT A.PLNO) AS RET_CTC_CNT
	FROM FDMTAG04001 A
	WHERE CLG_YM = SUBSTR(:ET_PARM02,1,6)
		AND EXISTS (SELECT 1 FROM VALID_PLNO B WHERE A.PLNO=B.PLNO)
	GROUP BY RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, NTNL_CTRY_CD, ARC_INPL_CD, INER_CHN_DVCD;
...
WITH VALID_PLNO AS (
	SELECT PLNO
		...
		WHERE CLG_YM = SUBSTR(:ET_PARM02,1,6)
		GROUP BY PLNO
		HAVING SUM(T02_LTPD_RN_URND_PRM) > 0
)
...
-- 일반_출재_미경과보험료
SELECT RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, NTNL_CTRY_CD, ARC_INPL_CD, INER_CHN_DVCD, T02_RN_RINSC_CD,
       SUM(T02_LTPD_RN_URND_PRM) AS T02_LTPD_RN_URND_PRM
	FROM FDMTAG04002
	WHERE CLG_YM = SUBSTR(:ET_PARM02,1,6)
	GROUP BY RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, NTNL_CTRY_CD, ARC_INPL_CD, INER_CHN_DVCD, T02_RN_RINSC_CD;

-- 일반_원수_예정보험료
SELECT RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, ARC_INPL_CD, NTNL_CTRY_CD, INER_CHN_DVCD, SUM(WNCR_CNV_EPCT_PRM) AS WNCR_CNV_EPCT_PRM
	FROM (
  	...
  	SELECT DISTINCT PLNO, ARC_INPL_CD, RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, NTNL_CTRY_CD, INER_CHN_DVCD
    	FROM FDMTAG04001
  ) B
  ON A.PLNO=B.PLNO
  GROUP BY RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, ARC_INPL_CD, NTNL_CTRY_CD, INER_CHN_DVCD;
...
SELECT B.ARC_INPL_CD, B.RRNR_CTC_BZ_DVCD, B.RRNR_DAT_DVCD, B.NTNL_CTRY_CD, B.INER_CHN_DVCD, A.T02_RN_RINSC_CD, SUM(A.WNCR_CNV_RN_PRM) AS WNCR_CNV_RN_PRM
	FROM EPCT_RN_PRM A
  LEFT JOIN (SELECT DISTINCT PLNO, ARC_INPL_CD, NTNL_CTRY_CD, RRNR_DAT_DVCD, RRNR_CTC_BZ_DVCD, INER_CHN_DVCD FROM FDMTAG04002) B
  	ON A.PLNO=B.PLNO
    GROUP BY B.ARC_INPL_CD, B.RRNR_CTC_BZ_DVCD, B.RRNR_DAT_DVCD, B.NTNL_CTRY_CD, B.INER_CHN_DVCD, A.T02_RN_RINSC_CD
