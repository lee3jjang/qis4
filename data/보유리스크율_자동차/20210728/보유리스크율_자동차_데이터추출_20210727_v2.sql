...
-- 자동차_특약보종별_직전1년경과보험료 (2021.07.28)
	...
	FROM (
		SELECT /*+ FULL(A) PARALLEL(A 8) */
		       SUBSTR(UY_YM,1,4) AS UY, BSC_CVR_CD, PDGR_CD,
			 		 SUM(ELP_PRM) AS OGL_ELP_PRM
			FROM FDMTAG02001 A
			INNER JOIN (SELECT /*+ FULL(E) PARALLEL(E 8) */ DISTINCT PLNO FROM FDMTAG02002 WHERE CLG_YM <= SUBSTR(:ET_PARM02,1,6)) E
				ON A.PLNO = E.PLNO
			WHERE CLG_YM <= SUBSTR(:ET_PARM02,1,6)
				AND SUBSTR(UY_YM,1,4) >= TO_CHAR(ADD_MONTHS(TO_DATE(:ET_PARM02, 'YYYYMMDD'),-59),'YYYY')
			GROUP BY SUBSTR(UY_YM,1,4), BSC_CVR_CD, PDGR_CD
	) C INNER JOIN (
		SELECT /*+ FULL(B) PARALLEL(B 8) */
		       SUBSTR(UY_YM,1,4) AS UY, BSC_CVR_CD, PDGR_CD,
