...
def clsf_cntr_catg_cd(data: pd.DataFrame, cntr_grp_info: pd.DataFrame) -> pd.Series:
    ...
    catr_catg_cd = data \
        ...
        .assign(CNTR_CATG_CD = lambda x: np.where((x['DMFR_DVCD'] == '02') & (x['RRNR_DVCD'] == '01'), '미국·캐나다', x['CNTR_CATG_CD']))
...
def clsf_boz_cd(data: pd.DataFrame, prd_info: pd.DataFrame) -> pd.Series:
    ...
    boz_cd = data \
        ...
        .assign(BOZ_CD = lambda x: np.where(x.PDC_CD.isin(['10011', '10012', '10013', '10016', '10017', '10580', '10900', '10901']), 'A003', x['BOZ_CD']))
